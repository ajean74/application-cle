...
<body class="body">
    <div class="text-center container-fluid">
        ...
        <div class="container">
            <form class="box-update2 arrondi2"  action="<?php echo isset($_GET['update']) ? '' : 'attente.php' ?>" method="POST" enctype="multipart/form-data" id="myForm">
                ...
                if(isset($_POST['valider'])){
                    ...
                    if(isset($_GET['update'])){
                        $id = $_GET['update'];
                        $stmt = $db->prepare("UPDATE Cle SET NomPersonne = :_nmpersonne WHERE id= :_id;");
                        $stmt->bindParam(':_nmpersonne', $nmpers);
                        $stmt->bindParam(':_id', $id);
                        $stmt->execute();
                        
                        $stmt = $db->prepare("UPDATE Cle SET Date = :_date WHERE id= :_id;");
                        $stmt->bindParam(':_date', $date);
                        $stmt->bindParam(':_id', $id);
                        $stmt->execute();
                        
                        $stmt = $db->prepare("UPDATE Cle SET Contact = :_contact WHERE id= :_id;");
                        ...
                        $stmt = $db->prepare("UPDATE Cle SET Note = :_note WHERE id= :_id;");
                        $stmt->bindParam(':_note', $note);
                        $stmt->bindParam(':_id', $id);
                        $stmt->execute();

                        $stmt = $db->prepare("UPDATE Cle SET Utilisateur = :_nmcollabo WHERE id= :_id;");
                        ...
                        if(isset($_POST['definitif'])){
                            ...
                            $stmt = $db->prepare("UPDATE Cle SET DateRetour = :_dateRetour WHERE id= :_id;");
