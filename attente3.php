...
<body class="body">
    <div class="text-center container-fluid">
        ...
        <div class="container">
            <form class="box-update2 arrondi2"  action="<?php echo isset($_GET['update']) ? '' : 'attente3.php' ?>" method="POST" enctype="multipart/form-data" id="myForm">
                ...
                if(isset($_POST['valider'])){
                    ...
                    $headers .= 'Cc: '.$copie."\n";   
                    $message = "La clé : ".$mailnum." vient d'être mis en transit.";
                    ...
                    if(isset($_GET['update'])){
                        $id = $_GET['update'];
                        $stmt = $db->prepare("UPDATE CleTransaction SET NomPersonne = :_nmpersonne WHERE id= :_id;");
                        $stmt->bindParam(':_nmpersonne', $nmpers);
                        $stmt->bindParam(':_id', $id);
                        $stmt->execute();
                        
                        $stmt = $db->prepare("UPDATE CleTransaction SET Date = :_date WHERE id= :_id;");
                        $stmt->bindParam(':_date', $date);
                        $stmt->bindParam(':_id', $id);
                        $stmt->execute();
                        
                        $stmt = $db->prepare("UPDATE CleTransaction SET Contact = :_contact WHERE id= :_id;");
                        ...
                        $stmt = $db->prepare("UPDATE CleTransaction SET Note = :_note WHERE id= :_id;");
                        $stmt->bindParam(':_note', $note);
                        $stmt->bindParam(':_id', $id);
                        $stmt->execute();

                        $stmt = $db->prepare("UPDATE CleTransaction SET Utilisateur = :_nmcollabo WHERE id= :_id;");
                        ...
                        if(isset($_POST['definitif'])){
                            ...
                            $stmt = $db->prepare("UPDATE CleTransaction SET DateRetour = :_dateRetour WHERE id= :_id;");
