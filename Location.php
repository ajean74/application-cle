...
<body class="body">
    <div class="text-center container-fluid">
        ...
        <div class="container">
            <form class="box-update arrondi2"  action="<?php echo isset($_GET['update']) ? '' : 'Location.php' ?>" method="POST" enctype="multipart/form-data" id="myForm">
                ...
                <div class="row mx-5 my-3 justify-content-center">
                    ...
                    <div class="col-md-4 p-4">
                        <h5>Locataire :</h5>
                        <input type="text" class="form-control" id="client2" name="client2" value="<?php echo isset($_GET['cl2']) ? $_GET['cl2'] : '';?>">
                    </div>
                </div>
                ...
                <?php
                    if(isset($_POST['valider'])){
                        ...
                        if(isset($_GET['update'])){
                            $id = $_GET['update'];
                            if($_GET['cle'] != $cle){
                                $stmt = $db->prepare("UPDATE CleLocation SET NumCle = :_numcle WHERE id= :_id;");
                                $stmt->bindParam(':_numcle', $cle);
                                $stmt->bindParam(':_id', $id);
                                $stmt->execute();
                            }
                            if($_GET['ncop'] != $ncop){
                                $stmt = $db->prepare("UPDATE CleLocation SET Copro = :_copro WHERE id= :_id;");
                                $stmt->bindParam(':_copro', $ncop);
                                $stmt->bindParam(':_id', $id);
                                $stmt->execute();
                            }
                            if($_GET['cl'] != $cl){
                                $stmt = $db->prepare("UPDATE CleLocation SET Proprietaire = :_client WHERE id= :_id;");
                                $stmt->bindParam(':_client', $cl);
                                $stmt->bindParam(':_id', $id);
                                $stmt->execute();
                            }
                            if($_GET['cl2'] != $cl2){
                                $stmt = $db->prepare("UPDATE CleLocation SET Locataire = :_client2 WHERE id= :_id;");
                                $stmt->bindParam(':_client2', $cl2);
                                $stmt->bindParam(':_id', $id);
                                $stmt->execute();
                            }
                            if($_GET['ag'] != $ag){
                                $stmt = $db->prepare("UPDATE CleLocation SET Agence = :_agence WHERE id= :_id;");
                            ...
                            if($oldpj != $newpj AND $newpj != NULL) {
                                $pj = $newpj;
                                $stmt = $db->prepare("UPDATE CleLocation SET Photo = :_pj WHERE id= :_id;");
                        ...
                        /* -- Creer -- */
                            ...
                            $stmt = $db->prepare("INSERT INTO CleLocation(NumCle, Copro, Proprietaire, Locataire, Agence, Etat, Photo) VALUES (:numcle, :copro, :client, :client2, :agence, :status, :pj);");
                            $stmt->bindParam(':numcle', $cle);
                            $stmt->bindParam(':copro', $ncop);
                            $stmt->bindParam(':client', $cl);
                            $stmt->bindParam(':client2', $cl2);
                        ...
                        /* -- Redirection -- */
                        if(isset($_GET['update'])){
                            echo '<script type="text/javascript">window.location.replace("Manager/clelocation.php");</script>';
                        }
                        else{
                            echo '<script type="text/javascript">window.location.replace("Manager/clelocation.php");</script>';
                        }
